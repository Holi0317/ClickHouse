...
{
    size_t size_limit = main_priority->getSizeLimit(lock);
    size_t elements_limit = main_priority->getElementsLimit(lock);
...
void FileCache::freeSpaceRatioKeepingThreadFunc()
{
    ...
    while (true)
    {
        ...
        try
        {
            ...
            auto eviction_candidates = main_priority->collectCandidatesForEviction(keep_up_free_space_remove_batch, stat, lock);
            ...
            {
                /// Impossible in realistic setup case, we should always be able to evict something.
            ...
            LOG_TRACE(log, "Current usage {}/{} in size, {}/{} in elements count "
                    ...
                    main_priority->getSize(lock), main_priority->getSizeLimit(lock),
                    main_priority->getElementsCount(lock), main_priority->getElementsLimit(lock),
                    keep_current_size_to_max_ratio, keep_current_elements_to_max_ratio,
